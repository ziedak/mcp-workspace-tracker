...
export class HttpTransport implements ITransportAdapter {
	...
	async connect(server: McpServer): Promise<void> {
		...
		if (this.config.cors) {
			this.app.use((req, res, next) => {
				if (this.config.cors) {
					...
					res.header("Access-Control-Allow-Headers", this.config.cors.allowedHeaders.join(", "));
				}
				next();
			});
		}
		...
		this.httpServer = this.app.listen(this.config.port, this.config.host || "localhost", () => {
			this.connected = true;
			console.info(`server up on ${this.config.host || "localhost"}:${this.config.port}`);
		});
