...
describe.skip("Transport Integration", () => {
	// Increase timeout for integration tests
	jest.setTimeout(15000);

	let tempDir: string;
	let workspaceTracker: IMcpWorkspaceTracker;
	let activeTransports: Array<{ disconnect: () => Promise<void>; isConnected?: () => boolean }> =
		[];
	...
	beforeEach(async () => {
		...
		// Get workspace tracker from container
		workspaceTracker = container.get<IMcpWorkspaceTracker>(TYPES.McpWorkspaceTracker);
		await workspaceTracker.initialize(tempDir);
	...
	describe("Start MCP Server Integration", () => {
		it("should start MCP server with stdio transport", async () => {
			const result = await startMcpServer(workspaceTracker, "stdio");
		...
		it("should start MCP server with default transport when none specified", async () => {
			const result = await startMcpServer(workspaceTracker);
		...
		it.skip("should start MCP server with HTTP transport", async () => {
			...
			const result = await startMcpServer(workspaceTracker, "http", {
				http: {
		...
		it("should handle transport configuration errors", async () => {
			await expect(
				startMcpServer(workspaceTracker, "http") // Missing required HTTP config
	...
	describe("End-to-End Workflow", () => {
		it("should complete full workspace analysis and server startup", async () => {
			...
			expect(workspaceTracker.getLogger()).toBeDefined();
			expect(workspaceTracker.getConfig()).toBeDefined();
