...
describe("start-server CLI Functions - Simple Unit Tests", () => {
	...
	describe("showHelp", () => {
		it("should display help message", () => {
			...
			expect(helpMessage).toContain("MCP Workspace Tracker Server");
	...
	describe("parseArguments - Simple Cases", () => {
		...
		it("should parse custom host", () => {
			const result = parseArguments(["--transport", "http", "--host", "0.0.0.0"]);
	...
	describe("parseArguments - Help Cases", () => {
		it("should show help and exit with --help", () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining("MCP Workspace Tracker Server")
			);
	...
	describe("parseArguments - Error Cases", () => {
		...
		it("should throw error for missing transport value", () => {
			expect(() => parseArguments(["--transport"])).toThrow("Error: --transport requires a value");
		});
		...
		it("should throw error for missing port value", () => {
			expect(() => parseArguments(["--port"])).toThrow("Error: --port requires a value");
		});

		it("should throw error for missing host value", () => {
			expect(() => parseArguments(["--host"])).toThrow("Error: --host requires a value");
		});
	...
	describe("parseArguments - Edge Cases", () => {
		...
		it("should handle complex workspace paths", () => {
			...
			expect(result.workspacePath).toBe("/path/with spaces/and-dashes_and.dots");
		});
